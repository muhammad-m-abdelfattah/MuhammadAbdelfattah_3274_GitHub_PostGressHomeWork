...
public class InvoiceFormatter {
	...
	public String format() {
		...
		String r = "                        I N V O I C E\n\n" + address.getName() +
				"\n" + address.getStreet()+ "/n" + address.getCity() + ", " + address.getState() +
				" " + address.getZip() + "\n" + String.format("\n\n%-" + DESCRIPTION_WIDTH + "s%" +
		...
		for (int i = 0; i < invoice.getLineItemCount(); i++) {
			...
			r += String.format("%-" + DESCRIPTION_WIDTH + "s%" + PRICE_WIDTH + "s%" + QTY_WIDTH
					+ "s%" + TOTAL_WIDTH + "s", item.getDescription(), item.getPrice(),
